...
apt update -y
apt download --target-release 'c=external' nvidia-driver-"$DRIVER"
...
sed -i "s#$(cat ./nvidia-driver-"$DRIVER"/DEBIAN/control | grep "Version: ")#$(cat ./nvidia-driver-"$DRIVER"/DEBIAN/control | grep "Version: ")-pika2.lunar#" ./nvidia-driver-"$DRIVER"/DEBIAN/control
